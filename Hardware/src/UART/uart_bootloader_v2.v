...
module uart_bootloader(
    ...
    always @(posedge clock) begin
        BootTimedEnable <= (reset) ? 29'h00000000 : (BootTimedEnable != 29'h1dcd6500) ? BootTimedEnable + 1'b1 : BootTimedEnable; // 5 sec @ 100 MHz
        BootSwEnabled <= (reset) ? 1'b1 : ((state == WRITE) ? ~DisableBoot : BootSwEnabled);
    end
    ...
    always @(posedge clock) begin
        rx_count <= (state == IDLE) ? 18'h00000 : ((state == XADDRI) ? rx_count + 1'b1 : rx_count);
    end
